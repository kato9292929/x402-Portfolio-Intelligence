...
import { handleHistory, readJsonBody } from "@/lib/multichain";
...
/**
 * POST /api/portfolio/history/polygon — Polygon USDC + JPYC ($0.30).
...
async function routeConfig(req: NextRequest): Promise<RouteConfig> {
  ...
  if (token === "jpyc") {
    return {
      price: {
        amount: "45000000000000000000", // 45 JPYC (18 decimals)
        asset: {
      ...
      network: "polygon",
      config: {
        description: "90日間のポートフォリオ履歴分析（Polygon JPYC決済）",
  ...
  return {
    ...
    network: "polygon",
    config: {
      description: "90日間のポートフォリオ履歴分析（Polygon USDC決済）",
      mimeType: "application/json",
    },
  };
}
