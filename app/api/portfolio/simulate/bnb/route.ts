...
import { handleSimulate, paymentChallenge, readJsonBody } from "@/lib/multichain";
...
/**
 * POST /api/portfolio/simulate/bnb — BNB Chain USDT, manual x402 ($0.50).
 * withX402 cannot serve BNB: x402-next 1.2.0 has no "bnb" network.
 * Body: { currentPortfolio, proposedChanges }
 */
export async function POST(req: NextRequest): Promise<NextResponse> {
  const challenge = paymentChallenge(req, {
    ...
    description: "リバランスシミュレーション（BNB Chain USDT決済）",
  });
