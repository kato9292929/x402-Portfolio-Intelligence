...
import { handleAnalyze, readJsonBody } from "@/lib/multichain";
...
/**
 * POST /api/portfolio/analyze/polygon — Polygon USDC + JPYC ($0.50).
...
async function routeConfig(req: NextRequest): Promise<RouteConfig> {
  ...
  if (token === "jpyc") {
    return {
      ...
      network: "polygon",
      config: {
        description: "ポートフォリオ分析（Polygon JPYC決済）",
  ...
  return {
    ...
    network: "polygon",
    config: {
      description: "ポートフォリオ分析（Polygon USDC決済）",
      mimeType: "application/json",
    },
  };
}
